...
class UsersTable extends Table
{
	...
	public function validationUpdateProfile()
    {
        ...
        ->add('full_name', 'unique', ['rule' => 'validateUnique', 'provider' => 'table','message' => 'Name is already exists.Please login with your exists account']);
	...
	public function fullnameUniqueCheck($value,$context){
		$full_name	 	 =	$context['data']['full_name'];
		$CasinoImage	 =	$this->find('all')->where(array('full_name' => $full_name,'is_deleted' => 0))->first();
		if(empty($CasinoImage)){
			return true;
		}
		return false;
	...
	public function validationSignUpForm()
    {
		...
		$validator
            ...
            ->notEmpty('full_name',__('Please enter full name.',true))
            ->add('full_name',[
				'fullnameUniqueCheck'=>[
					'rule'	  	=>  'fullnameUniqueCheck',
					'provider' 	=>  'table',
					'message'	=>	'Name already exists.Please login with your exists account'
				 ]
			]);;
