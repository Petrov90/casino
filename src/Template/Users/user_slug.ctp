...
width: 70% !important;
	...
	function initialize(title,address,htmladr) {
		...
		locations.push([title,address,htmladr]);
...
function myJsFunc4() {
			...
			$.ajax({
				url  	 : '<?php echo $this->Url->build('/messages/add'); ?>',
				data 	 : {receiver_id : id,message : u_msz},
				type	 : 'POST',
				dataType : 'json',
...
?>style="background-image:url(<?php echo WEBSITE_URL.'image.php?height=470px&cropratio=3:1&image='.PROFILE_IMG_URL.$userDetails->cover_image; ?>)"<?php
	...
	<div class="profileCounter">
		<div class="container"> <?php 
			if($userDetails->id == $this->request->session()->read('Auth.User.id')){ ?> <?php echo $this->Form->create('User',['url' => '/globalusers/upload_image_cover','id' => 'upload_img_form_cover']) ?>
			...
			<div class="profileBox">
				<div class="pull-left">
			   		<a class="pro_img_change" href="javascript:void(0);">
			   			<img src="<?php echo $url; ?>" alt="img"  id="image_src" class="img-responsive" />
			   		</a><?php 
			   	if($userDetails->id == $this->request->session()->read('Auth.User.id')){ ?>
			   		<div class="pro_file_img">
				  		<div class="fileUpload">
					 		...
					 		<span>Upload</span><?php 
					 		echo $this->Form->create('User',['url' => '/globalusers/upload_image','id' => 'upload_img_form']) ?><?php 
					 		echo $this->Form->file('image',['class' => 'upload','id' => 'upload_img_form_dile']) ?><?php 
					 		echo $this->Form->end(); ?>
				  		</div>
			   		</div><?php 
			   	} ?>
				...
				if($userDetails->id != $this->request->session()->read('Auth.User.id')){ ?>
					<div class="profileFlow"> <?php 
						if( (in_array('who_can_follow_me_all_users',$preference) && !empty($this->request->session()->read('Auth.User.id')))  || in_array('who_can_follow_me_anyone',$preference) || (in_array('who_can_follow_me_i_follow_him',$preference) && (!empty($userDetails->he_follow_m_e)))){ ?>
							<div class="block">
								<a data-isPopup="yes" href="#" data-id="<?php echo $userDetails->id; ?>" data-rel="<?php echo (!empty($userDetails->user_follower)) ? 'no' : 'yes'; ?>" class="btn red_btn follow" onclick="myfollow(); return false;">
									<i class="fa fa-plus <?php echo (!empty($userDetails->user_follower)) ? 'hide' : ''; ?>" id="f_icon"></i>
									<span id="f_text"><?php echo (!empty($userDetails->user_follower)) ? 'Following' : 'Follow'; ?></span>
								</a>	
							</div><?php 
						}
						if(in_array('who_can_contact_me_anyone',$preference) || (in_array('who_can_contact_me_all_users',$preference) && !empty($this->request->session()->read('Auth.User.id'))) || (in_array('who_can_contact_me_followers',$preference) && (!empty($userDetails->user_follower))) || (in_array('who_can_see_things_on_my_profile_i_follow_him',$preference) && (!empty($userDetails->he_follow_m_e)))){ ?>
							<div class="block">
								<a href="#" class="btn red_btn message" onclick="myJsFunc3(); return false;"><i class="fa"><img src="<?php echo WEBSITE_IMG_URL ?>ic6.png" alt="icon" /></i> Message</a>					
							</div><?php 
						} ?>
            		</div><?php
            	} ?>
      		...
      		<div class="profileTag">
         		<div class="container">
		            ...
		            <div class="col">
		               <a class="followerList" href="#" onclick="followerList(); return false;"><i><img src="<?php echo WEBSITE_IMG_URL ?>ic10.png" alt="icon" /></i> Followers <span id="like_counter">(<?php echo $userDetails->follower_count; ?>)</span></a>
		            </div>
         		</div>
      		</div>
   		</div>
	</div>
	<div class="mid_wrapper"><?php 
		if( in_array('who_can_see_things_on_my_profile_anyone',$preference) || (in_array('who_can_see_things_on_my_profile_followers',$preference) && (!empty($userDetails->user_follower))) || (in_array('who_can_see_things_on_my_profile_all_users',$preference) && (!empty($this->request->session()->read('Auth.User.id')))) ||($userDetails->id == $this->request->session()->read('Auth.User.id')) ){ ?>

   			<div class="CasinoVisited">
      			<div class="container">
         			<h2 class="litelTitle">Casino I Visited</h2><?php 
         			if(!$casinoIvisites->isEmpty()){  ?>
 					<div class="mapLoction">
    					<div class="mapListcol">
       						<div class="listarrow">
       							<a href="javascript:void(0);" id="downClick" class="fa fa-chevron-up"></a>
       						</div>
       						<ul id="scrl"><?php 
		 						if(!$casinoIvisites->isEmpty()){ 
									foreach($casinoIvisites as $key => $casino){
										 if($key == 0){
											$title = $casino->casino->title;
											$address = $casino->casino->address;
											$htmlurl1 =  $this->Url->build(['plugin' => '','controller' => 'casinos','action' => 'casino_view','casino_view'=>$casino->casino->slug]);
										}
										$htmlurl =  $this->Url->build(['plugin' => '','controller' => 'casinos','action' => 'casino_view','casino_view'=>$casino->casino->slug]);
										?>
								<li>
									<div class="pull-left">
										<a href="javascript:void(0);" dt-hrf="<?php echo $htmlurl?>" class="nwtb_cas" onclick="initialize('<?php echo isset($casino->casino->title) ? str_replace("'","",$casino->casino->title) : ''; ?>','<?php echo isset($casino->casino->address) ? str_replace("'","",$casino->casino->address) : ''; ?>','<?php echo isset($htmlurl) ? str_replace("'","",$htmlurl) : ''; ?>')">
										 <?php if(!empty($casino->casino->image) && file_exists(CASINO_FULL_IMG_ROOT_PATH.$casino->casino->image)){ ?>
												<?php echo $this->Html->image(WEBSITE_UPLOADS_URL.'image.php?width=70px&height=70px&cropratio=1:1&image='.CASINO_FULL_IMG_URL.$casino->casino->image,['alt'=>$casino->casino->title.' review', 'class' => 'img-responsive']); ?>
											<?php 
											}else{ ?>				
												<img alt="<?php echo $casino->casino->title.' review'?>" src="<?php echo WEBSITE_URL.'image.php?width=70px&height=70px&cropratio=1:1&image='.NO_CASINO_IMG; ?>" class="img-responsive" />
											<?php } ?> 
										</a>
									</div>
									<div class="locatnDet">
										<a href="javascript:void(0);" dt-hrf="<?php echo $htmlurl?>" class="nwtb_cas" onclick="initialize('<?php echo isset($casino->casino->title) ? str_replace("'","",$casino->casino->title) : ''; ?>','<?php echo isset($casino->casino->address) ? str_replace("'","",$casino->casino->address) : ''; ?>','<?php echo isset($htmlurl) ? str_replace("'","",$htmlurl) : ''; ?>')"><?php echo isset($casino->casino->title) ? $casino->casino->title : ''; ?></a>
											<div class="rating">
					  ...
					  <div class="col">
						 <div class="pull-left">
							...
							<?php if(!empty($result->casino->image) && file_exists(CASINO_FULL_IMG_ROOT_PATH.$result->casino->image)){ ?>
								<?php echo $this->Html->image(WEBSITE_UPLOADS_URL.'image.php?width=70px&height=70px&cropratio=1:1&image='.CASINO_FULL_IMG_URL.$result->casino->image,['alt'=>$result->casino->title.' review','class' => 'img-responsive']); ?>
							<?php 
							}else{ ?>				
								<img alt="<?php echo $result->casino->title.' review'?>" src="<?php echo WEBSITE_URL.'image.php?width=70px&height=70px&cropratio=1:1&image='.NO_CASINO_IMG; ?>" class="img-responsive" />
							<?php } ?></a>
						</div>
			...
			<div id="message-form_error_div"></div>
               <div class="fildlogin">
                  <div class="form-group">
					...
					<textarea class="form-control login-field" id="u_msz" name="message" rows="5"></textarea>
					<!-- <?php //echo $this->Form->hidden('receiver_id',['value' => $userDetails->id]); ?> -->
					<input name="receiver_id" id="receiver_id" value="<?php echo $userDetails->id;?>" type="hidden">
                  </div>
               </div>
               <div class="loginfooter">
					<input type="submit" value="Submit" class="btn red_btn send_message" data-rel="message-form" onclick="myJsFunc4(); return false;"/>
               </div>
...
//echo $this->Html->script(['https://maps.googleapis.com/maps/api/js?v=3&libraries=geometry,places&key&language=en&key=AIzaSyCPU4hzQzGaRkgV-3K9yN_aSwL_KsDBh1A'],array('block' =>'custom_script'));
	...
	<?php if(!empty($this->request->session()->read('Auth.User'))){ ?>
		btn 	=	$(".follow");
		rel		=	btn.attr('data-rel');
		id		=	btn.attr('data-id');
		isPopup		=	btn.attr('data-isPopup');
		if(btnclick){
			btnclick = false;
