...
<?php if(in_array('who_can_see_things_on_my_profile_all_users',$preference) || (in_array('who_can_see_things_on_my_profile_followers',$preference) && (!empty($userDetails->user_follower))) || (in_array('who_can_see_things_on_my_profile_all_users',$preference) && (!empty($this->request->session()->read('Auth.User.id')))) ||($userDetails->id == $this->request->session()->read('Auth.User.id')) ){ ?>
