                      ...
                      <a href="<?php echo $this->Url->build(['plugin' => '','controller' => 'users','action' => 'guide_view','guide_view' => $contents->slug]); ?>">
                          <?php 
                              if(!empty($contents->image2) && file_exists(GALLERY_ROOT_PATH.$contents->image2)){
                                 echo $this->Html->image(WEBSITE_URL.'image.php?width=95px&height=95px&cropratio=1:1&image='.GALLERY_IMG_URL.$contents->image2,['class' => 'img-responsive','alt' =>$contents->image_alt]);
                              } ?>
                      </a>
                    </div>
