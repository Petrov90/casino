			...
			foreach($images as $key => $img){
			 ...
			 ?>
				<li	<?php if($key > 0){ ?>style="display:none" <?php  } ?>>
					...
					<?php }else{ */?>
							<img alt="img"  id="<?php echo $key; ?>_img" class="<?php echo $bglow_cls; ?> lazy" src="<?php echo WEBSITE_IMG_URL.'camera-loader.gif' ?>" data-src="<?php echo CASINO_FULL_IMG_URL.$img->file ?>" />
					<?php /*}*/ ?>
