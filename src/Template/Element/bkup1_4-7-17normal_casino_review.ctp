		...
		<?php } if($k){	?>
			<div class="member_detail">			
         		<span class="span-member">
					...
					<h4>This casino has no review yet</h4> 
				</span>
				<div class="member_detail">
				 	...
				 	<div class="memberRiver addQuiton">
						...
						<div id="review_form1_error_div"></div>
							...
							<div class="revi_memb revi_memb1 rv_mem2">
								<!--<?php/* echo $this->Form->textarea('comment',['class' => 'form-control','placeholder' => 'Be the first to write a reivew']);*/ ?>-->

								<textarea id="txt_rvw" name="comment" class="form-control" placeholder="Be the first to write a reivew" rows="5" <?php if(empty($this->request->session()->read('Auth.User'))) { echo 'readonly';}?>></textarea>
							</div>
...
<?php } ?>
	...
	$(document).on('click','#txt_rvw1',function(){
	<?php if(empty($this->request->session()->read('Auth.User'))){ ?>
			$(".login-pop").trigger('click');
		<?php }?>
	}); 
	$(document).on('click','#txt_rvw',function(){
	<?php if(empty($this->request->session()->read('Auth.User'))){ ?>
			$(".login-pop").trigger('click');
		<?php }?>
	});	
