...
<script>
	$(document).on('click','.show_all_btn1',function(){
			...
			if(atrb_anstxt === "Show all answers") {
				$("#hide_show_ans").attr("anstxt","Hide all answers");
				$(hidn_div).show();
				$("#hide_show_ans").text("Hide all answers");
			} else {
				$("#hide_show_ans").attr("anstxt","Show all answers");
				$(hidn_div).hide();
				$("#hide_show_ans").text(show_ans_txt);
			}
	});
</script>