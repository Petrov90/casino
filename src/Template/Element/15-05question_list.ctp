	...
	$url1	=	WEBSITE_URL.'image.php?width=150px&height=150px&cropratio=1:1&image='.NO_PROFILE_IMAGE.$sex.'.png';
         ...
         <div class="col-md-7">
            <div class="contri_text contri_text1">
               <h3><?php echo $review->created->format('M,d Y'); ?></h3>
               <p><?php echo nl2br($review->comment); ?></p>
            </div>
            <div class="show_all_btn">
               <?php if(count($review->question_comments) > 0){ ?><a onclick="$('#an<?php echo $review->id; ?>').show()" href="javascript:void(0);">Show all answers(<?php echo count($review->question_comments); ?>)</a><?php } ?>
            </div>
         </div>	  
         <div class="col-md-3">
		...
		<?php foreach($review->question_comments as $question_comments){ ?>
			<div class="filter_post_item">
				 ...
				 <div class="col-md-9">
					<div class="contri_text contri_text1">
					   <h3><?php echo $question_comments->created->format('M,d Y'); ?></h3>
