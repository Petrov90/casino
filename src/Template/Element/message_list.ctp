				...
				foreach($allMessage as $message){
					...
					<tr>
						<td>
							...
							<a data-type="<?php echo $type; ?>" href="javascript:void(0)" data-sender="<?php echo $message->sender_id;?>"data-id="<?php echo $message->id; ?>" class="message_view">
							<!-- <?php //echo ($type == 'inbox') ? $message->sender->full_name.' ('.$message->sender->email.')' :  $message->receiver->full_name.' ('.$message->receiver->email.')' ; ?> -->
							<?php echo ($type == 'inbox') ? $message->sender->full_name :  $message->receiver->full_name; ?>	</a>
						</td>
		...
		<?php echo $this->Form->create('User',['url' => '/messages/add','id' => 'reply-form','class' => 'css-form']); ?>
			...
			<div id="rplyBox" class="block hide">
				<textarea name="message" placeholder="Reply" id="reply_box" rows="5"></textarea>
				 <button id="rplymsg" class="btn red_btn message_reply pull-right" data-rel="reply-form" type="submit">
					<?php echo 'Answer'; ?>
				 </button>
			</div>
			<?php } ?>
	...
	$(".autocomplete1").autocomplete({
		source: function( request, response ) {
		  ...
		  return $( "<li'>" )
			...
			.append("<a href='javascript:void(0)'>" +name+"</a>")
			.appendTo( ul );
	};
