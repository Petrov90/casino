...
<div id="rw_div" name="rw_div" class="member_review" ng-app="comment" ng-controller="commentController">
	<div class="<?php if($count) echo 'member_bord';?>" id="rw_brd" name="rw_brd">
		<?php if($count) { ?>
		<div class="member">

		<h2 class="mainheading"><?php echo ($count == 0) ? 'Users Reviews' : 'Member Reviews'; ?> (<span id="count_review"><?php echo $count; ?></span>)</h2>
	  </div>
		...
		foreach($reviewList as $reviews){ $k = 0; ?>
			<div class="member_detail" id="comment_div_<?php echo $reviews->id; ?>">
				...
				<div class="memberRiver">
					...
					<p class="readmoretext"><span class="fa fa-quote-left qt_lft">	</span>
						<?php echo nl2br($reviews->comment); ?><span class="fa fa-quote-right qt_rt">
					</span>
					</p>
					</div>
					<div class="cment_sect">
						<img src="<?php echo WEBSITE_IMG_URL ?>cment.png" class="img-responsive" alt="img" />
						<a href="javascript:void(0);" data-id="addcomment_<?php echo $reviews->id; ?>" cmntid="<?php echo $reviews->id; ?>" class="show_comment">Add Comment</a><span>-  Was this  review helpful ?</span>
						<div class="play_yes">
						...
						<?php }
						if($reviews->comment_count > 0){ ?>
								<a href="javascript:void(0);" class="show_all_review_comments" id="shw_ans_<?php echo $reviews->id;?>" anstxt="Show all answers"  divid="<?php echo $reviews->id; ?>" ans_no="<?php echo $reviews->comment_count;?>">Show all answers(<qa id="<?php echo $reviews->id; ?>_showall" data-count="<?php echo (!empty($reviews->comment_count)) ? $reviews->comment_count : 0 ;  ?>"><?php echo (!empty($reviews->comment_count)) ? $reviews->comment_count : 0 ;  ?></qa>)</a><?php
							} ?>
						...
						<?php } ?>
						<div class="comment_boxN"  style="display: none;" id="show_comment_<?php echo $reviews->id; ?>">
							<div class="CommentRow" id="comment_box_<?php echo $reviews->id; ?>">
								<?php //pr($reviews);
								if (!empty($reviews->review_comments)){
									foreach($reviews->review_comments as $key => $comments){
									/*if($key > 3){continue;}  */
		...
		<?php } if($k){	?>
			<div class="member_detail">
         		<span class="span-member">
         			<img src="<?php echo WEBSITE_URL.'webroot/img/first_review.png' ?>" alt="img">
					<h4>This <?php echo $type;?> has no reviews yet</h4>
				</span>
				<div class="member_detail">
				 	<div class="member_col">
				 		<div class="mem_img">
				 			<img src="<?php echo WEBSITE_URL.'image.php?width=150px&height=150px&cropratio=1:1&image='.NO_PROFILE_IMAGE.'m_f.png';; ?>" alt="img" class="img-responsive man"/>
				 		</div>
				 	</div>
				 	<div class="memberRiver addQuiton">
						<?php echo $this->Form->create('User',['url' => array('plugin' => '','controller' => 'users','action' => 'addReview'),'id' => 'review_form1']); ?>
						<?php
						echo $this->Form->hidden('foreign_key',['id' => 'foreign_key']);
						echo $this->Form->hidden('type',['id' => 'type']); ?>
						<div id="review_form1_error_div"></div>
							<div class="memberRiver addQuiton">
							<div class="revi_memb revi_memb1 rv_mem2">
								<!--<?php/* echo $this->Form->textarea('comment',['class' => 'form-control','placeholder' => 'Be the first to write a reivew']);<?php //if(empty($this->request->session()->read('Auth.User'))) { echo 'readonly';}?>
								*/ ?>-->

								<textarea id="txt_rvw" name="comment" class="form-control" placeholder="Be the first to write a reivew" rows="5"></textarea>
							</div>
							<button type="submit" class="r-btn review_submit1">Add Review</button>
							<div class="reviewBtn rvmdl_bd" id="txt_rvw1">
								&nbsp;
								<div class="modal-body md_bdy">
					<!-- <?php //if(!empty($this->request->session()->read('Auth.User'))) {?> -->
								<div class="jrating111" data-score="0">
						...
						<img alt="1" src="../images/star-off.png" title="Bad" />&nbsp;
  							...
  							<img alt="5" src="../images/star-off.png" title="Great" />
								</div>
						    <?php }*/ ?> -->
				 	...
				 	<!-- 	<div id="" class="revi_memb revi-memb-height">
				 		<p id="" class="">Be the first to write a reivew</p> -->
				 	</div>
				 	<?php /*
				 		<div class="rerpo_btn respond respondBTN2">

							<div class="AddReviewBtn hide add-rvw-btn">
								<div class="pull-left">
									<a class="btn-no-rv-rt">
									<?php for ($im=0 ; $im<5 ;$im++) {?>
					  				<img src="<?php echo WEBSITE_URL.'webroot/images/star-o.png' ?>"><?php }?>
									</a>
								</div>
								<div class="pull-right">
								<a href="javascript:void(0);" data-name="<?php echo $name; ?>" data-type="<?php echo $type; ?>" data-id="<?php echo $foreign_key; ?>" class="btn red_btn city-click btn_new">Add Review</a>
								</div>
							</div>
						</div>
				 	*/?>
	...
	$(document).on('click', '.rvew-btn-cancel', function(){
	 ...
	 $(document).on('click', '.show_comment', function(){
		...
		<?php }else{ ?>
			...
			$("#shw_ans_"+cmntid).attr("anstxt","Hide all answers");
			$("#shw_ans_"+cmntid).text("Hide all answers");
			$("#show_comment_"+cmntid).show();
			$("#"+id).show('slow');
			$('html, body').animate({
				...
				<div class="modal-body">
					<form action="<?php echo $this->Url->build('/users/report_as_spam'); ?>" id="report_span_form" accept-charset="utf-8" method="post" class="ng-pristine ng-valid">
					  ...
					  <div id="report_span_form_error_div"></div>
					  	<button type="button"  class="btn info_btn " data-dismiss="modal" >Cancel</button>
...
<div class="modal fade writeReview" id="succ_popup1" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
...
<script>
	...
	$(".review_submit1").click(function(e){


		e.preventDefault();
		form_id	=	'review_form1';
