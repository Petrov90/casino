...
<?php /*
	  <div class="sort_search"><span>Sort By :</span>
		...
		$ques_uid = array();
		$show_ques_btn = 1;
		$rv_shw_ans = $reviewList;
		foreach($rv_shw_ans as $reviews) {
			$ques_uid[] = $reviews->user->id;
		 }
		$log_uid = $this->request->session()->read('Auth.User.id');
		if(in_array($log_uid, $ques_uid)) {
			$show_ques_btn = 0;
		}
		// code for hiding add question after adding on question

	  ?>

<div class="member_review">
	<div class="<?php if($count) echo 'member_bord';?>">
	  <div class="member">
	  	<?php if($count) { ?>
		<h2><?php echo ($type == 'news') ? 'Comments' : 'Questions & Answers'; ?> (<span id="count_question"><?php echo $count; ?></span>)</h2><?php } ?>
	  </div>
	</div>
		<?php
	if($count) {
...
<form action="<?php echo $this->Url->build(WEBSITE_URL.'questions/add'); ?>" id="question_form" accept-charset="utf-8" method="post" >
<?php
			...
			<div class="member_detail" id="comment_div_<?php echo $reviews->id; ?>">
				...
				<div class="memberRiver addQuiton">
					<div class="revi_memb" id="questionsppp<?php echo $reviews->id; ?>">
						...
						<p class="readmoretext" id="questionstext<?php echo $reviews->id; ?>"><?php echo $reviews->comment; ?></p>
					</div>
					...
					<?php if($reviews->comment_count > 0){ ?>
						<div  class="allAnswer">
							<a href="javascript:void(0);" divid="<?php echo $reviews->id; ?>" id="" anstxt="Show all answers" class="show_all_comments" data-id="<?php echo $reviews->id; ?>" ans_no="<?php echo count($reviews->question_comments)?>" >Show all answers(<?php echo (!empty($reviews->comment_count)) ? '<span id="count_answer_'.$reviews->id.'">'.$reviews->comment_count.'</span>' : 0 ;  ?>)</a>
						</div><?php
					}
					if (!empty($reviews->question_comments)) { ?>
						<div style="display: none;" id="comment_div_all_<?php echo $reviews->id; ?>"><?php
						foreach($reviews->question_comments as $key => $comments) {
							/*if($key > 3) {
								continue;
							}*/
		...
		<?php } if($k){	?>
			<div class="member_detail" id="frst_askr">
				...
				<div class="member_detail">
				 	...
				 	<div class="memberRiver addQuiton" id="AddCommentPopup1">
						<form action="<?php echo $this->Url->build(WEBSITE_URL.'questions/add'); ?>" id="question_form1" accept-charset="utf-8" method="post" >
						<?php
						echo $this->Form->hidden('foreign_key',['id' => 'foreign_key1']);
						echo $this->Form->hidden('type',['id' => 'type1']); ?>
						<div id="question_form1_error_div"></div>
				   		<div class="memberRiver addQuiton">
							<div class="revi_memb revi_memb1 rv_mem2">
							<textarea name="comment" class="form-control" placeholder="Be the first to ask a question" rows="5" <?php //if(empty($this->request->session()->read('Auth.User'))) { echo 'readonly';}?>></textarea>
							<!-- 	<?php /*echo $this->Form->textarea('comment',['class' => 'form-control','placeholder' => 'Be the first to ask a question']);*/ ?> -->
							</div>
							<div class="reviewBtn">
								<button type="submit" class="r-btn comment_submit1"><?php echo ($type == 'news') ? 'Add Comment' : 'Add Question'; ?></button>&nbsp;
								<!-- <button type="button"  class="w-btn">Cancel</button> -->
							</div>
				   		</div>
				 		...
				 		<p id="" class="">Be the first to ask a question</p> -->
				 	</div>
				 		<?php /*
				 		<div class="rerpo_btn respond respondBTN2">

							<div class="AddReviewBtn  add-rvw-btn">
							<div class="pull-left">
...
<script type="text/javascript">
	...
	$(".comment_submit1").click(function(e){
			e.preventDefault();
			form_id	=	'question_form1';
		<?php if(empty($this->request->session()->read('Auth.User'))){ ?>
			$("#ajax-loader").addClass('hide');
			$(".login-pop").trigger('click');
		<?php } else {?>
			...
			var options = {
				...
				success:function(r){
					...
					if(data.success){
						...
						var error_div_id	=	form_id+'_error_div';
						 $('#AddCommentPopup1').animate({ scrollTop: 0 }, 'slow');
				...
				<div class="modal-body">
					<form action="<?php echo $this->Url->build('/question/report_as_spam'); ?>" id="report_span_modal_question" accept-charset="utf-8" method="post" class="ng-pristine ng-valid">
					  ...
					  <div class="reviewBtn">
						...
						<input type="submit" class="btn red_btn report_span_update_question" value="Report" />
...
<script>
	$(document).on('click','.show_all_comments',function(){
			...
			hidn_div = "#comment_div_all_"+divid;
