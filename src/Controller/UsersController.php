...
class UsersController extends AppController
{
	...
	public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(array('locations','index','contactUs','promotion','addReview','changeLang','cms','cityView','cityCasino','cmsSlug','cronFunction','login','like','signup','cityAutocomplete','casinoSearch','test11','fbsignup','Facebook','countryView','facebook2','userSlug','follow','forgotPassword','resetPassword','getReviews','topUsers','newReviews','captcha','guide','article','guideView','continenent','africa','asia','europe','northAmericaAndCaribbean','southAmerica','australia','casinoName','listfollow','updtCntry','reportAsSpam','demo'));
    }
    ...
    public function index(){
		...
		/* if (($allBlocks = Cache::read('allBlocks','longlong')) === false) { */
			...
			$allBlock = $this->Blocks->find('translations')->where(['Blocks.id IN (3,4,5,6,7,8,25,27,28,29,30,31)'])->toList();
		...
		/* if (($popularCasinos = Cache::read('popularCasinos','longlong')) === false) { */
			...
			$popularCasinos = $this->PopularCasinos->find('all')->contain(['Casinos' => ['fields' => ['slug','type','country_id','review_count','avg_rating']],'Casinos.City' => ['fields' => ['name']] ])->where(['isfeat' => 1])->limit(6)->order('rand()')->toList();
			//pr($popularCasinos);die;
			/* Cache::write('popularCasinos', $popularCasinos,'longlong'); */
		/* } */
		$cntId = array();
			foreach ($popularCasinos as $key=>$v) {
				$cntId[] = $v->casino->country_id;//$cntId[] = $v['country_id'];
			}
		$cntByCas = $this->cntryByCasino($cntId);

		$worldPopularCasino	= $this->casinoByContinent();
		$wordCasino = $worldPopularCasino;
		$cityname = array();
		foreach ($wordCasino	 as $continent) {
			foreach($continent->casinos as $c) {
				$cityname[] = $c->city_name;
			}
		}
		$cityname = array_unique($cityname);
		$worldPopularcities			=	$this->citiesByContinent($cityname);
		$newsList = $this->newsList();

	   // print_r($newsList);
	   $newsId = array();
			foreach ($newsList as $key=>$v) {
				$newsId[] = $v->id;//$cntId[] = $v['country_id'];
			}

		$newsCmnt = $this->newsComment($newsId);


		$pageTitle			=	__('title.homepage');
		$metaDescription	=	__('metadescription.homepage');
		$this->set(compact('sliders','promotions','reviewList','popularCasinos','allBlocks','pageTitle','metaDescription','continents','worldPopularCasino','worldPopularcities','newsList','cntByCas','newsCmnt'));
    }

    function cntryByCasino($cntId) {
		$cntar =array();
		$conn =  ConnectionManager::get('default');
		foreach ($cntId as $key =>$v) {
			$query = "Select countries.name from countries
				 where  countries.id in (".$v.")  " ;

			$cntry = $conn->execute($query);
			$cntry = $cntry ->fetchAll('assoc');
			$cntar[] = $cntry[0]['name'] ;
		}
	...
	function newsComment($newsId) {
		$newsar =array();
		$conn =  ConnectionManager::get('default');
		foreach ($newsId as $key =>$v) {
			$query = "Select questions.comment_count from questions
				 where  questions.type='news' and questions.foreign_key in (".$v.")  " ;

			$news = $conn->execute($query);
			$news = $news ->fetchAll('assoc');
			//pr($news);die;
			if(!empty($news[0]['comment_count'])) {
				$newsar[] = $news[0]['comment_count'] ;
			}else {
				$newsar[] = 0 ;
			}
	...
	function casinoByContinent(){
		...
		$worldPopularCasino			=	$this->Continents->find('all')->contain(['Casinos' => ['conditions' => ['is_feature' => 1],'fields' => ['title','continent_id','city_name','slug']]])->all();
		return $worldPopularCasino;
	}
	function citiesByContinent($cityname){
	    return false;
		/*$this->loadModel('CityManager.Continents');
		$query = "Select cities.name,countries.continent_id, cities.slug as city_slug, countries.slug as country_slug 	from countries
				inner join cities on cities.country_id = countries.id where ( cities.name in ".$cityname." ) order by countries.continent_id" ;*/
		$cityname ="('" . implode("', '", $cityname) . "')";
		$conn =  ConnectionManager::get('default');
	$query = "Select cities.name,countries.continent_id, cities.slug as city_slug, countries.slug as country_slug 	from countries
				inner join cities on cities.country_id = countries.id
				 where ( cities.casino_count >0  and cities.name in ".$cityname." ) order by countries.continent_id " ;
		$cities = $conn->execute($query);
		$worldPopularcities = $cities ->fetchAll('assoc');
	    return $worldPopularcities;
	}
	...
	public function locations(){
		...
		$this->loadModel('CityManager.City');
			$featureCity  = $this->City->find('all')
	        ->where([
	          'City.is_feature' => 1
	        ])->order(['City.id' => 'desc'])->limit(6);

	    $this->loadModel('Casinos');
		$totalCasino	= $this->Casinos->find('all')
				->where([
					'Casinos.type' => 'normal'
				])->all();
		$review_count = $totalCasino->sumOf('review_count');
		$question_count = $totalCasino->sumOf('question_count');
		$totalCasino		=	$totalCasino->count();
		...
		$this->set(compact('pageTitle','metaDescription','headBlock','cities','Continents','worldPopularCasino','worldPopularcities','totalCasino','featureCity','review_count','question_count','promotions','popularCasinos','cntByCas'));
    }
	...
	$this->loadModel('CityManager.Country');
	$cityDetail = $this->Country->find('all')->where(['Country.slug' => $countrySlug])->first();
...
$order = 'asc';
		...
		$this->paginate = [
			'sortWhitelist' => ['Casinos.id','Casinos.avg_rating','Casinos.title'],
			'limit' => Configure::read('Reading.record_per_page'),
			'group' => 'Casinos.id'
		];
		
		$query->where(['Casinos.country_id' => $cityDetail->id])->order(['Casinos.avg_rating' => 'desc','Casinos.title' => 'asc']);
		$result		 		= 	$this->paginate($query);
		//echo "<pre>"; print_r($result); die;

		$this->set(compact('cityDetail','result'));
		$slugName	=	'country_view';
		$this->set(compact('slug','slugName'));
		
        $this->set('limit',10);    
            
            
            
            $this->render('country_view_2');
        }else{
       ...
       $cityDetail = $this->City->find('all')->contain([
		...
		if($cityDetail){
			...
			$slugName			=	'city_view';
			$this->loadModel('Casinos');
			$query = $this->Casinos->find()->order(['Casinos.avg_rating' => 'desc','Casinos.title' => 'asc']);
			$this->paginate = [
			...
			$query->where(['Casinos.city_id' => $cityDetail->id])->order(['Casinos.avg_rating' => 'desc','Casinos.title' => 'asc']);
			$casinosData = $this->paginate($query);
		 //echo "<pre>";print_r($casinosData->count());
            ...
            $lat		=	$cityDetail->latitude;
			$lang		=	$cityDetail->longitude;
			$this->loadModel('Casinos');
			...
			$countryCity = $this->City->find('all')->where(['City.country_id' => $cityDetail->country_id])->order(['City.id' => 'desc'])->all();
...
// 			$countryCity = $this->City->find('all')->select([$distance])->select($this->City)->where(['City.country_id' => $cityDetail->country_id])->order(['City.id' => 'desc'])->all();

			$this->set(compact('cityDetail','slug','slugName','images','pageTitle','metaDescription','recommedCasinos','casinosData','countrySlug','countryCity'));
			$this->set('_serialize', ['cityDetail','images']);
			//echo "<pre>";
			//print_r($casinosData); die;

		}
        }
	}

	public function cityCasino($slug = ''){
	...
	public function cityAutocomplete(){
		...
		if(strpos($query,'__')){
			...
			$results['City'] 	= $this->City->find('translations')
					...
					->order(['City.casino_count' => 'desc','City.name' => 'asc'])
					->limit(10);
	...
	function countryView($slug = null){
		...
		$this->loadModel('CityManager.City');
		$myallCity =$this->City->find('all')->where(['City.country_id' => $countryDetail->id,'City.casino_count !=' => 0])->order(['City.casino_count' => 'desc','City.name' => 'asc'])->limit(80)->toList();
		...
		$query->where(['Casinos.country_id' => $countryDetail->id,'Casinos.review_count !=' => 0])->order(['Casinos.avg_rating' => 'desc','Casinos.review_count' => 'desc','Casinos.title' => 'asc']);
		$casinosData = $this->paginate($query);
		...
		//echo '<pre>';print_r($casinosData);die;
		$this->set(compact('countryDetail','slug','slugName','pageTitle','metaDescription','casinosData','recommnedCasino','images','myallCity'));
		// pr($images);
		...
		if ($this->request->is('post')) {
		    if(isset($this->request->data['picture']) && !empty($this->request->data['picture'])){
        		...
        		if(isset($this->request->data['email']) && !empty($this->request->data['email'])){
        		    ...
        		    $name =  $this->request->data['name'];
        		    $id =  $this->request->data['id'];
        		    //echo $email.'+'.$name.'+'.$id; die;
        		    $users	=	$this->Users->find('all')->where(['gplus_id' => $id])->first();
        		    if(empty($users)){
            			$user = $this->Users->newEntity();
	...
	function facebook(){
		...
		$fb = new \Facebook\Facebook([
			  'app_id' => '1872420869440519',
			  'app_secret' => '97e73d90d3fa03f98fd11a6014e15487',
		...
		$loginUrl = $helper->getLoginUrl('http://www.casinoo.com/users/facebook2', $permissions);
...
/* 	function article(){
	...
	function guideView($slug = null)
    {
		...
		$GuideContents1 = $this->GuideContents->find('translations')->where(['isfeat' => 1,'is_deleted' => 0])->limit(5);

		// $headBlock = $this->Blocks->find('translations')->where(['Blocks.id' => 24])->first();
		//echo "<pre>";
		//print_r($guideContent);
		$this->set(compact('slugPass','result','slug','slugName','GuideContents1','headBlock'));
	}
    ...
    public function updtCntry($slug=null,$id=null,$img=null)
    {
/*    	echo $id;die;*/
    	$this->autoRender = false;

        $conn =  ConnectionManager::get('default');
		$query = "Update countries set cntry_img ='".$img."' where id = ".$id ;
		//echo '<pre>';print_r($query);
		$update_cntry = $conn->execute($query);
 /*       if ($id) {
			$updtcntry->id	=	$id;
			$updtcntry->cntry_img	=	$img;
            if ($this->Countries->save($updtcntry)) {
				$id	=	$updtcntry->id;
     ...
     public function newsList(){
		...
		return $newsList->order(['News.id' => 'desc'])->toList();
	}
