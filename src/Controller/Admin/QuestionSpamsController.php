...
/**
 * QuestionSpams Controller
 *
 * @property \App\Model\Table\QuestionSpamsTable $QuestionSpams
 */
class QuestionSpamsController extends AppController
{
    ...
    public function index()
    {
		...
		$this->paginate = [
				...
				'sort' => 'id',
				'direction' => 'desc',
				'contain' => ['Users', 'Questions','Questions.City','Questions.City.CCountry','Questions.Casinos','Questions.Country','Questions.Users','Master']
			];
		
        $QuestionSpams = $this->paginate($query);

        $this->set(compact('QuestionSpams'));
        $this->set('_serialize', ['QuestionSpams']);
    }
    ...
    {
        $reviewSpam = $this->QuestionSpams->get($id, [
    ...
    public function add()
    {
        $reviewSpam = $this->QuestionSpams->newEntity();
        if ($this->request->is('post')) {
            $reviewSpam = $this->QuestionSpams->patchEntity($reviewSpam, $this->request->data);
            if ($this->QuestionSpams->save($reviewSpam)) {
        ...
        $users = $this->QuestionSpams->Users->find('list', ['limit' => 200]);
        $reviews = $this->QuestionSpams->Reviews->find('list', ['limit' => 200]);
    ...
    public function delete($id = null)
    {
        ...
        $reviewSpam = $this->QuestionSpams->get($id);
        if ($this->QuestionSpams->delete($reviewSpam)) {
