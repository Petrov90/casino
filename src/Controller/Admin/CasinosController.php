 ...
 /**
	...
	public function casino_amount()
    {
       ...
       $query = $this->Amounts->find();
		...
		$this->paginate = ['sortWhitelist' => ['title'],'limit' => Configure::read('Reading.record_per_page'),
			'order' => [
				'Amounts.id' => 'desc'
			]];
		$query->order(['Amounts.id' => 'desc']);
		$casinos = $this->paginate($query);
    ...
    {
    	$this->loadModel('Amounts');
		$casino = $this->Amounts->newEntity();		
        if ($this->request->is('post')) {
			$casino 	= 	$this->Amounts->patchEntity($casino, $this->request->data);
			if(!$casino->errors()){
				
				$this->Amounts->save($casino);
				$this->Flash->success(__('The amount has been saved.'));
				return $this->redirect(['action' => 'casino_amount']);
	...
	public function casino_request_edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
			...
			if(!$casino->errors()){
				...
				return $this->redirect(['action' => 'casino_request']);
		...
		// $casinoActivities			 =	 $this->CasinoActivities->find('all', ['contain' => ['ChildMasters'],'conditions' => ['casino_id' => $casino->id,'parent_id' => 0,'is_deleted' => 0]]);
		$casinoActivities			 =	 $this->CasinoActivities->find('all', ['contain' => ['ChildMasters'],'conditions' => ['parent_id' => 0,'is_deleted' => 0]]);
		//echo "<pre>"; print_r($casinoActivities);
    ...
    public function casino_request_delete($id = null)
    {
	...
	public function casino_reques($id = null,$type = null)
    {
        $this->request->allowMethod(['post', 'delete']);
        ...
        if ($this->Casinos->save($casino)) {
            ...
            $this->Flash->error(__('The casino could not be updated. Please, try again.'));
        }
    ...
    public function add()
    {
		...
		$this->set(compact('casino','amenities','gambling_options','country','city','casinoActivities'));
        $this->set('_serialize', ['casino']);
		$this->set('model',$this->modelClass);
    }
...
public function stnd(){
        if ($this->request->is(['patch', 'post', 'put'])) {
			
				$this->Flash->success(__('Standard Image has been updated.'));
				return $this->redirect(['action' => 'stnd']);
