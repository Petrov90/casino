...
class SocialShareHelper extends Helper
{
    ...
    public function faqcategory($id) {
        $articles = TableRegistry::get('FaqQuestions');
        $query = $articles->find('all',['conditions' => array('category_id' => $id)]);    
        return $query;
    }
    public function countryslug($id) {
        $articles = TableRegistry::get('CityManager.Country');
        $query = $articles->find('all')->where(['Country.id' => $id])->first();
        //$query = $articles->find('all',['conditions' => array('Country.id' => $id)]);    
        return $query;
    }
    public function casinopromotions($id) {
        $articles = TableRegistry::get('Promotion.Promotions');
        $query = $articles->find('all')->where(['Promotions.casino_id' => $id])->first();
        //$query = $articles->find('all',['conditions' => array('Country.id' => $id)]);    
        return $query;
    }
    public function casinodetails($id) {
        $articles = TableRegistry::get('Casinos');
        $query = $articles->find('all')->where(['Casinos.id' => $id])->first();
        //$query = $articles->find('all',['conditions' => array('Country.id' => $id)]);    
        return $query;
    }
    public function get_meters_between_points($lat1, $lon1, $lat2, $lon2) {
    if (($lat1 == $lat2) && ($lon1 == $lon2)) { return 0; } // distance is zero because they're the same point
    $p1 = deg2rad($lat1);
    $p2 = deg2rad($lat2);
