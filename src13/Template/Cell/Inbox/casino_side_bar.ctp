		 ...
		 <?php foreach($recommedCasinos as $casino){ ?>
			<div class="item_D1">
			   ...
			   <div class="item_text">
				  ...
				  <small class="readonly" data-score="<?php echo $casino->avg_rating; ?>"></small>
					<a <?php echo NEWTAB ?> data-title="<?php echo $casino->title ?>" data-url="<?php echo $casino->slug ?>" rel="nofollow" class="btn playBtn" href="<?php  echo $this->Url->build(array('plugin' => '','controller' => 'users','action' => 'casino_name' ,'casino_name' => $casino->main_promotion->slug)); ?>">Play now</a>
			   </div>
			</div>
		 <?php } ?>
		 </div>
		 ...
		 <?php foreach($latestBonuses as $bonus){  ?>
			<div class="item_D1">
			   ...
			   <div class="item_text">
				  ...
				  <small class="readonly" data-score="<?php echo $bonus->casino->avg_rating; ?>"></small><a data-title="<?php echo $bonus->casino->title ?>" data-url="<?php echo $bonus->casino->slug ?>"  <?php echo NEWTAB ?> rel="nofollow" class="btn playBtn" href="<?php  echo $this->Url->build(array('plugin' => '','controller' => 'users','action' => 'casino_name' ,'casino_name' => $bonus->slug)); ?>">Play now</a>
			   </div>
			</div>
		 <?php } ?>
		 </div>
