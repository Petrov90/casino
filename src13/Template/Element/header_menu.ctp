...
/* if(in_array($controller,array('Casinos')) && in_array($action,array('addCasino','casinoView','onlineCasinoView')) || in_array($controller,array('Globalusers')) && in_array($action,array('index')) ||  in_array($controller,array('Users')) && in_array($action,array('addReview','cmsSlug','contactUs'))){
	...
	<div class="container">
	   <div class="row">
		  <div class="logo">
				<a href="<?php echo $this->Url->build(array('plugin' => '','controller' => 'users','action' => 'index')); ?>"><img src="<?php echo WEBSITE_URL.'image.php?width=210px&height=45px&cropratio=4:1&image='.WEBSITE_URL."webroot/img/newlogo.png"; ?>" alt="Img" class="img-responsive" /></a>
		  </div>
