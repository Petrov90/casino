...
<div class="masters view large-9 medium-8 columns content">
    ...
    <div class="related">
        <h4><?= __('Related Casino Amenities') ?></h4>
        <?php if (!empty($master->casino_amenities)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($master->casino_amenities as $casinoAmenities): ?>
            <tr>
                <td><?= h($casinoAmenities->id) ?></td>
                <td><?= h($casinoAmenities->casino_id) ?></td>
                <td><?= h($casinoAmenities->master_id) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['controller' => 'CasinoAmenities', 'action' => 'view', $casinoAmenities->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['controller' => 'CasinoAmenities', 'action' => 'edit', $casinoAmenities->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'CasinoAmenities', 'action' => 'delete', $casinoAmenities->id], ['confirm' => __('Are you sure you want to delete # {0}?', $casinoAmenities->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
    ...
    <div class="related">
        <h4><?= __('Related Casino Gambling Options') ?></h4>
        <?php if (!empty($master->casino_gambling_options)): ?>
        <table cellpadding="0" cellspacing="0">
            <tr>
                <th><?= __('Id') ?></th>
                <th><?= __('Casino Id') ?></th>
            ...
            <?php foreach ($master->casino_gambling_options as $casinoGamblingOptions): ?>
            <tr>
                <td><?= h($casinoGamblingOptions->id) ?></td>
                <td><?= h($casinoGamblingOptions->casino_id) ?></td>
                <td><?= h($casinoGamblingOptions->master_id) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['controller' => 'CasinoGamblingOptions', 'action' => 'view', $casinoGamblingOptions->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['controller' => 'CasinoGamblingOptions', 'action' => 'edit', $casinoGamblingOptions->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'CasinoGamblingOptions', 'action' => 'delete', $casinoGamblingOptions->id], ['confirm' => __('Are you sure you want to delete # {0}?', $casinoGamblingOptions->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
