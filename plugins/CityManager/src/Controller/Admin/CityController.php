...
class CityController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) { 
			...
			if(!$city->errors()){
				...
				$City_name		=	$this->City->find('all')->where(array('country_id' => $this->request->data['country_id'],'name' => $this->request->data['_translations']['en']['name']))->order(['id' => 'ASC'])->first();
				$City_slug		=	$this->City->find('all')->where(array('country_id' => $this->request->data['country_id'],'slug' => $this->request->data['slug']))->order(['id' => 'ASC'])->first();
				//echo $City_name->name;
				//pr($this->request->data); //die("kp");
				if(isset($City_name->name) && !empty($City_name->name)){
					$this->Flash->error(__('City already exists.Please enter another city.'));
					return $this->redirect(['action' => 'add']);

				}if(isset($City_slug->slug) && !empty($City_slug->slug)){
					$this->Flash->error(__('City slug already exists.Please enter another city.'));
					return $this->redirect(['action' => 'add']);

				}else{
					//pr($this->request->data); die("kp");
					$city->name	=	$this->request->data['_translations']['en']['name'];
					$city->object_id	=	$this->request->data['object_id'];
					$city->slug	=	$this->request->data['slug'];
					$city->first_head	=	$this->request->data['first_head'];
				    $city->first_block	=	$this->request->data['first_block'];
				    $city->second_blok	=	$this->request->data['second_blok'];
					$this->City->save($city);
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
			...
			if(!$city->errors()){
				...
				$City_name		=	$this->City->find('all')->where(array('country_id' => $this->request->data['country_id'],'name' => $this->request->data['_translations']['en']['name'],'id !=' => $city->id))->order(['id' => 'ASC'])->first();
				$City_slug		=	$this->City->find('all')->where(array('country_id' => $this->request->data['country_id'],'slug' => $this->request->data['slug'],'id !=' => $city->id))->order(['id' => 'ASC'])->first();
				if(isset($City_name->name) && !empty($City_name->name)){
					$this->Flash->error(__('City already exists.Please enter another city.')); 
					//return $this->redirect(['action' => 'add']);

				}elseif(isset($City_slug->slug) && !empty($City_slug->slug)){
					$this->Flash->error(__('City slug already exists.Please enter another city.'));
					//return $this->redirect(['action' => 'add']);
    ...
    public function feat($id = null,$status = null)
    {
    	$casino =	$res	= $this->City->get($id);
		 
		$casino->is_feature	=	$status;		
		$this->City->save($casino);		
