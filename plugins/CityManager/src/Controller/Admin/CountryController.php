    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
			...
			if(!$country->errors()){
				...
				$country->head_frst	=	$this->request->data['head_frst'];
				$country->object_id	=	$this->request->data['object_id'];
				$this->Country->save($country);
				$this->Flash->success(__('The country has been saved.'));
				return $this->redirect(['action' => 'index']);
			}
		...
		$qry = $this->CasinoImages->find('all', $options)->order(['id' => 'asc']);
		$qry = $qry->all();
		$c_images = $qry->toArray();

		$Continents	=	$this->Country->Continents->find('list');		
		
        $this->set(compact('country','lanagauageList','Continents','c_images'));
        $this->set('_serialize', ['country']);
    /* 
