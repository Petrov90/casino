				...
				<div id="<?php echo $lanagauage->code; ?>_div" class="tab-pane <?php echo ($lanagauage->is_default == 1) ? 'active' : '';  ?>">
					<div class="col-lg-12">
						...
						<div class="form-group">
							...
							<?php echo $this->Form->text("head_frst",array('class' => 'form-control','required' => false));
							echo $this->Form->error("head_frst"); ?>
						</div>
						...
						<div class="form-group" style="">
							<label>Second blok</label>
							<?php 
							$val123 		=	 (!empty($country->_translations[$code]->second_blok)) ? $country->_translations[$code]->second_blok : $country->second_blok;
							  echo $this->Form->textarea('_translations.'.$code.".second_blok",array('class' => 'form-control','placeholdder' => 'Second blok','value'=>$val123,'id' =>'bodya'.$code)); 
							  $this->Html->scriptStart(array('inline' => false,'block' => 'custom_script')); ?>
								CKEDITOR.replace( 'bodya<?php echo $code; ?>',{} );
							<?php $this->Html->scriptEnd(); 
							echo ($lanagauage->is_default == 1) ? $this->Form->error("second_blok") : ''; ?>
						</div>	
					</div>
				</div>
			<?php } ?>
			<div class="col-lg-12">
					...
					<label>Image</label>
					 <div id="container">
						...
						if(!empty($country->object_id)){
							...
							$object_id = rand(999999999,mt_getrandmax());
						}
						$user_id = ADMIN_ID;   
						$type_id = 1;
				...
				<div class="cnt_img form-group">
					...
					<ul class="files"><?php
						...
						if( in_array($country->cntry_img, $images_arr)){
							$dbimg = 1;
						}
						$img_id = ''; 
						...
						foreach($c_images as $img){ 
							if($dbimg && ($country->cntry_img == $img->file) ){
									$img_id = 'mn_img';
							}

							?>
							<li class="dflt_cntry_img " itmid="<?php echo $country->id;?>" imgid="<?php echo $img->id?>"itm_name="<?php echo $img->file;?>" slgname="<?php echo $country->slug?>" id="<?php echo $img_id; ?>"><img alt="img" src="<?php echo WEBSITE_URL.'image.php?width=150px&height=75px&cropratio=2:1&image='.CASINO_FULL_IMG_URL.$img->file ?>" />
...
$(document).ready(function() {
	$(".dflt_cntry_img").click(function(){
		...
		if($(imgstyl).css('display') == 'block')
		{
			$('[id^=icn_]').css("display","none");
			img_name = '';
			msz = 'Country image has been Unmarked';
		} else {
			$('[id^=icn_]').css("display","none");
			$(imgstyl).css("display","block");
			msz = 'Country image has been marked';
		}
